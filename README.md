...
SemVer tags reachable from `main` trigger the release workflow after a full source verification gate. Backend and public-web images are published to GHCR with BuildKit provenance/SBOM attestations, scanned with Trivy, keyless-signed with Cosign using GitHub OIDC and recorded in an immutable-digest `release-manifest.json` plus checksum. Release/deployment JSON Schemas document the evidence contracts, and promotion adds semantic tag/source/repository checks. A separate GitHub Environment-gated promotion workflow accepts only an environment and tagged release version, derives the source revision/digests from the published release asset, verifies mainline reachability, tag-to-commit binding and exact tag-bound Cosign identities, then produces an environment deployment manifest. Provider adapters deploy `ref@sha256:digest` coordinates without rebuilding.
