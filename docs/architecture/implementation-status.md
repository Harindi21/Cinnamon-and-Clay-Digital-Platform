...
| Delivery pipeline | Complete for provider-neutral portfolio scope | Production container builds are PR-validated; mainline SemVer releases publish GHCR images, BuildKit provenance/SBOM attestations, CycloneDX SBOMs, Trivy gates, keyless Cosign signatures and checksummed immutable release manifests. Versioned JSON Schemas document release/deployment evidence; GitHub Environment promotion resolves coordinates from the published tagged release, verifies mainline/tag/source/ref integrity plus exact tag-bound signatures, and emits evidence-rich deployment manifests. A cloud/provider deployment adapter remains environment work. |
