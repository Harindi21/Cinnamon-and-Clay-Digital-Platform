...
4. Set alternative text, placement, display order and active state.
5. Save.
...
First verify `/api/v1/media` returns the new media `version`. The Next.js media URL includes that version for cache busting. If the public metadata itself is stale, the remaining public-content revalidation window is a separate cache-invalidation concern tracked in the implementation backlog.
