...
The Android build contract is intentionally explicit: `flutter_secure_storage` 11.x requires API 37, and Android SDK Manager publishes that platform as `platforms;android-37.0` rather than `platforms;android-37`. The app therefore declares `release(37)` with `minorApiLevel = 0`; `targetSdk` remains Flutter-managed so a compile-time compatibility update does not silently opt the app into newer runtime behavior. Android Gradle Plugin 9.1.1 is paired with Gradle 9.3.1, an API-37.0-supported combination. CI provisions SDK Platform 37.0 and AGP 9.1.1's supported/default Build Tools 36.0.0 before building, validates the toolchain contract, and validates the Gradle distribution URL and SHA-256 in `android/gradle/wrapper/gradle-wrapper.properties`. The Flutter tool materializes its SDK-managed Gradle wrapper launcher/JAR during Android builds, so CI does not invoke `gradlew` as a standalone preflight.

## Quality checks

```powershell
.\tool\ensure_android_sdk.ps1
flutter pub get
.\tool\verify_gradle_wrapper_pin.ps1
.\tool\verify_android_toolchain.ps1
flutter analyze
flutter test
